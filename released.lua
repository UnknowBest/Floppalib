...
local function getObjGen()
    ...
    local function getObjects()
        local function initObj()
            ...
            Gui.Logo.Size = UDim2.new(1, 0, 0.15, 0)
            Gui.Logo.ZIndex = 122
            Gui.Logo.Image = "rbxassetid://95976497861614"
    ...
    function objGen.new(objectType, cheatName)
        if objectType == "Cheat" then
            if UIObjects.Cheats:FindFirstChild(cheatName) then
                return UIObjects.Cheats[cheatName]:Clone()
            else
                error("Invalid cheatType")
            end
        end

        if UIObjects.Objects:FindFirstChild(objectType) then
            return UIObjects.Objects[objectType]:Clone()
        else
...
function UILibrary.Window:SetScale(scaleFactor)
    if self.MainUI.MainUI:FindFirstChild("UIScale") then
        self.MainUI.MainUI.UIScale.Scale = scaleFactor
    end
end

function UILibrary.Window:ToggleUI()
    self.MainUI.Enabled = not self.MainUI.Enabled
end
